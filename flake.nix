  ...
  outputs =
    ...
    let
      ...
      configfile = buildLib.buildKernelConfig {
        inherit (kernelConfig) generateConfigFlags structuredExtraConfig enableRust;
      ...
      initramfs = buildLib.buildInitramfs {
        ...
        } // pkgs.lib.optionalAttrs enableBPF { stackcount = "${pkgs.bcc}/bin/stackcount"; };
        storePaths =
          [ pkgs.foot.terminfo ]
      ...
      neovimPkg =
        (neovim-flake.lib.neovimConfiguration {
          inherit pkgs;
          modules = [ (pkgs.callPackage ./configs/editor.nix { inherit enableRust; }) ];
        }).neovim;
      ...
      rustModule = buildRustModule {
        ...
        src = if useRustForLinux then ./modules/rfl_rust else ./modules/rust;
      };
      ...
      devShell =
        ...
        pkgs.mkShell {
          ...
          shellHook =
            # bash
            ''
              echo "Welcome to a Linux module dev environment"
              PS1="Linux-dev: \w \$ "
